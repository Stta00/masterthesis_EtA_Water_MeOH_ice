...
# === Path & filenames (Ethanolamine) ===
folder = r"C:\Users\taras\OneDrive\Desktop\MA\Subtraction spectra\Ethanolamine"
basenames = [f"Subtractionspectra_ETA{i}" for i in range(1, 6)]
basenames.append("20240626_Deposition_ZnSe_20K_sample1")
ext_candidates = [".csv", ".txt", ".dat", ".dpt", ".asc"]

# If you also have an additional “main” spectrum like you had for Methanol, add it here, e.g.:
# basenames.append("20240626_Deposition_Ethanolamine_20K_sample1")

# === Plot x-window ===
x_left, x_right = 925, 820

# === Integration window ===
a, b = 900, 855  # adjust if needed

#eta_range_cm1 = (925.0, 820.0)
#eta_box_cm1   = (880.0, 855.0)
...
def read_two_columns(path):
    """
    Read (wn, y) from text-like files.
    - tries simple split strategies
    - fallback: regex that extracts first 2 numbers per line (handles decimal comma)
    """
...
print(f"Ethanolamine — Integration over {a}–{b} cm^-1 (relative to y=0):")
print("--------------------------------------------------------------")
...
# keep same y-window as before; adjust if needed
ax.set_ylim(0.125, 0.22)

ax.set_title("Ethanolamine spectra (same y-axis, no stacking) + integration area")
ax.invert_xaxis()
